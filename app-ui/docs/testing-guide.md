...
| 2026-07-19 | Teleport-stub corollary: a `wrapper.find(...)` element handle captured BEFORE a re-render (e.g. before `setValue`) can go stale — the stub recreated the node, so assertions on the old handle read frozen state. Re-find after every state change instead of caching the DOMWrapper | `wp39-duration-prices.spec.ts` |
| 2026-07-27 | `window.print()` flows: jsdom's `print` is "not implemented" (throws via virtual console) — assign `window.print = vi.fn()` in `beforeEach`. Anything true only *during* the print call (e.g. a body class that hides `#app` under `@media print`) must be captured INSIDE the print mock's implementation — it's torn down synchronously right after `print()` returns. Clean any body-class side effects in `afterEach` | `wp35-session-history-pack.spec.ts` |
| 2026-07-31 | Testing `HttpClientBase` itself (e.g. that a ProblemDetails `detail` string surfaces as the thrown `Error.message`): define a tiny local subclass exposing the protected verb (`class TestClient extends HttpClientBase { postTest(u){ return this.post(u, {}); } }`), `vi.stubGlobal('fetch', …)` returning a minimal fake `{ ok, status, text: async () => body } as unknown as Response`, and `vi.unstubAllGlobals()` in `afterEach`. No authBridge setup needed — the default bridge returns a null token | `wp42-cancellation-money-ux.spec.ts` |
